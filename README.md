...
Multi-agent web research for pi. Coordinate parallel researchers or run single-agent queries. Search the web, scrape URLs, query security databases, access Stack Exchange, search code, and track progress via terminal UI. Multiple search engines, no cost thanks to [SearXNG](https://github.com/searxng/searxng). 
