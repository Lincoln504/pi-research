...
// Build the PDF extraction worker bundle. Same lifecycle rules as the browser
// thread-worker: a pre-compiled .mjs next to its source, `packages: 'external'`
// so it resolves pdf-oxide-wasm from the installed node_modules, and never
// committed to git (regenerated by prepare).
function buildPdfWorker() {
  return esbuild.build({
    ...COMMON,
    entryPoints: [p('src', 'web-research', 'pdf-extract-worker.ts')],
    outfile: p('src', 'web-research', 'pdf-extract-worker.mjs'),
  });
}
...
async function ensurePdfWorkerBuilt() {
  const src = p('src', 'web-research', 'pdf-extract-worker.ts');
  const out = p('src', 'web-research', 'pdf-extract-worker.mjs');
  const stale =
    !fs.existsSync(out) || fs.statSync(out).mtimeMs < fs.statSync(src).mtimeMs;
  if (stale) {
...
// Copy the PDF worker bundle into dist/ next to the cli bundle; pdf-extraction.ts
// resolves it relative to its own compiled location at runtime.
function copyPdfWorker() {
  fs.mkdirSync(p('dist'), { recursive: true });
  fs.copyFileSync(
    p('src', 'web-research', 'pdf-extract-worker.mjs'),
    p('dist', 'pdf-extract-worker.mjs'),
  );
}

const TARGETS = {
  worker: () => buildWorker(),
  pdfworker: () => buildPdfWorker(),
  cli: async () => {
    await ensureWorkerBuilt();
    await ensurePdfWorkerBuilt();
    await esbuild.build({
      ...COMMON,
