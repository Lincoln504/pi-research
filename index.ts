...
export default function (pi: ExtensionAPI) {
  ...
  pi.on('before_agent_start', async (event) => {
    const researchMessage = [
      ...
      '',
      '**DEFAULT BEHAVIOR:** When you call the research tool, simply omit the `depth` parameter. It defaults to `0` (Quick mode) which is ideal for most queries.',
      '',
      '**DEPTH LEVELS:**',
      '- `depth: 0` (Quick/Brief — DEFAULT) — 1 researcher, 1 round. Fastest; use for simple questions or focused lookups.',
      ...
      'When you use the research tool, simply DO NOT specify a depth parameter in most cases. Let it default to 0.',
      'Only specify higher depths when the user explicitly requests thoroughness or the topic is complex.',
      '',
      '**WHEN TO USE HIGHER DEPTHS:**',
      '• Use `0` (omit the parameter) if the answer likely resides in 1-2 sources — MOST COMMON',
