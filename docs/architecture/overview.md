...
A high-level view showing the relationships between major architectural layers (`core`, `infrastructure`, `orchestration`, `tools`, `tui`, etc.). This graph is optimized for horizontal flow and is used to verify that the project follows a strict layered architecture where dependencies only flow downward.

### Detailed Module Graph

**File:** `docs/deps.svg`

A comprehensive, file-level dependency graph showing every module in the `src/` directory. This graph is automatically regenerated by CI on every push to the `main` branch, ensuring it always reflects the current state of the codebase.
