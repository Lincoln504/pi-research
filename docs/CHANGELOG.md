...
Clears the medium/low findings deferred out of the 1.1.2 audit. Every item below was
confirmed against the code — and, where the claim was about the operating system or a
package manager, against a live experiment on this machine rather than from memory.
...
### Changed
- **Cancelled runs exit `130` instead of `70`.** *This changes the CLI's exit-code contract.* A deliberate stop is not a runtime fault, but it shared `SOFTWARE` (70) with genuine crashes — and 70 is documented in `SKILL.md` as "runtime error — suggest one retry for transient failures", so an agent relaying the result would offer to re-run the very research the user had just interrupted. Ctrl-C, SIGTERM and programmatic aborts now exit `130` (the conventional 128+SIGINT), report "cancelled" rather than "failed", and set `cancelled: true` (never `retryable`) in `--json`. Classification keys off the signal handler having fired, not off pattern-matching whichever subsystem noticed the abort first. `SKILL.md` documents the new code.
...
- **The agent-skill launcher reported a cancelled run as a runtime error.** `run.mjs` maps a signal-killed engine to exit `70` — the code `SKILL.md` documents as "runtime error, suggest one retry" — so a harness that SIGTERMs a run on timeout prompted the agent to re-run research that had deliberately been stopped, one layer above the engine's own (now fixed) classification. `SIGINT`/`SIGTERM`/`SIGHUP`/`SIGQUIT` now exit `130` and report "cancelled"; `SIGKILL` and the rest still exit `70`, since an unexplained hard kill is more likely an OOM than a user's decision. Verified end-to-end against a stub engine: SIGTERM → `130`, SIGKILL → `70`.
