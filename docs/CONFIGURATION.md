...
| `CAMOUFOX_INSTALL_DIR` | _(user cache)_ | camoufox-js's own variable and the one that actually relocates the browser. Sets where the binary is downloaded, looked up and launched from; exported to the postinstall fetch and to browser workers so all three agree. |
| `PLAYWRIGHT_BROWSERS_PATH` | _(user cache)_ | Documented alias for the above — accepted for compatibility and exported onward as `CAMOUFOX_INSTALL_DIR`. On its own it never moved the download (camoufox-js below 0.12.0 honoured no variable at all), so setting it used to report the browser missing from the custom path after a successful install. |
