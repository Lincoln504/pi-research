...
export function getComplexityGuidance(complexity: 1 | 2 | 3, maxTeamSize: number, queryBudget: number): string {
  if (complexity === 1) {
    ...
    return `**Complexity: Level 3 (Ultra)**. Perform an exhaustive, deep-dive research effort, leaving no stone unturned. **IMPORTANT**: Plan aggressively for multiple research rounds with comprehensive citation throughout. In your initial planning, deploy up to ${maxTeamSize} researchers and fully utilize each researcher's query budget (${queryBudget}). Think in terms of a multi-phase investigation: plan Round 1 to broadly map the landscape with parallel specialists, anticipating that subsequent rounds will cover remaining gaps. Not every round needs all ${maxTeamSize} researchers — use as many as the topic scope requires, but be thorough.
...
**IMPORTANT**: Level 2 is designed for multi-round research. You should typically delegate for 2-3 rounds before considering synthesis. Each round adds value, depth, and citation diversity to your findings. Prioritize source coverage — aim for multiple authoritative references per topic. Be proactive with delegation — default to delegating when in doubt, rather than synthesizing with incomplete findings or insufficient citations. Scale researcher count to match the gaps — focused gaps may only need 1 researcher, while broad gaps benefit from the full team.`;
  } else {
...
Be aggressive with delegation. Level 3 is for exhaustive research — use remaining rounds to drill into specialized details, verify findings with additional sources, or explore nuanced dimensions. Scale the team size to match the scope of remaining gaps — a single focused researcher is fine for drilling into a specific detail, while broad unexplored areas warrant the full team.
...
export function getRoundPhaseGuidance(currentRound: number, maxRounds: number, complexity: 1 | 2 | 3, maxTeamSize: number): string {
  ...
  if (roundRatio <= 0.5) {
    ...
    if (complexity === 2) {
      return `\n\n---\n\n**Round Phase: MIDDLE (Round ${currentRound} of ${maxRounds})**\n\n**Level 2 (Deep) Guidance**: You are in the middle phase of deep research. Continue delegating actively — you should aim for 2-3 total rounds before synthesis. Each round adds value and depth to your findings. Keep delegating when there are still meaningful gaps or areas to explore.\n\n- Not every round needs all ${maxTeamSize} researchers — use as many as the remaining gaps require\n- Synthesize only when findings are comprehensive and no significant gaps remain that warrant another round.\n\n`;
    }
    // Level 1 middle
