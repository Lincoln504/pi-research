...
// Geometry is expressed in pulse frames. The pulse runs at ~30 FPS
// (33ms/frame, see pulse.ts) — one cell-step per frame, which is the natural
// visual resolution for a one-cell-wide head and reads cleaner than 60 FPS
// (see pulse.ts for why higher FPS adds jitter without smoothing the motion).
const DWELL_FRAMES = 10; // pause duration at each edge (~330ms at 33ms/frame)
const PERIOD_FRAMES = 98; // total frames for one full back-and-forth cycle (~3.2s at 33ms/frame)
